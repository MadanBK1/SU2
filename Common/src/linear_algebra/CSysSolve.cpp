...
#include "../../include/linear_algebra/KokkosVectorOps.hpp"
...
unsigned long CSysSolve<ScalarType>::FGMRES_LinSolver(const CSysVector<ScalarType>& b, CSysVector<ScalarType>& x,
  ...
  if (config->GetKokkos() && mat_vec.SupportsKokkosDeviceResident()) {
    ...
    for (i = 0; i < m; ++i) {
      ...
      /*--- SU2 CGS2 pass 1.  Compute all projection coefficients
       * before modifying w.  MultiDot performs one MPI Allreduce for the
       * complete coefficient vector. ---*/
      const auto h = KokkosLinearAlgebra::MultiDot(V, i + 1, V[i + 1]);
      for (unsigned long k = 0; k <= i; ++k) {
        H(k, i) = h[k];
      }
      for (unsigned long k = 0; k <= i; ++k) {
        KokkosLinearAlgebra::AXPY(V[i + 1], -h[k], V[k]);
      }

      /*--- SU2 CGS2 pass 2.  Again calculate every coefficient from the same
       * post-pass-1 vector before applying the second projection. ---*/
      const auto dh = KokkosLinearAlgebra::MultiDot(V, i + 1, V[i + 1]);
      for (unsigned long k = 0; k <= i; ++k) {
        H(k, i) += dh[k];
      }
      for (unsigned long k = 0; k <= i; ++k) {
        KokkosLinearAlgebra::AXPY(V[i + 1], -dh[k], V[k]);
      }
